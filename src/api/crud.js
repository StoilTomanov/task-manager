...
export async function login(userData) {
    try {
        ...
        const result = await response.json();
        setUserData(result.username, result.sessionToken, result.objectId, userData['isTeam']);
    } catch (err) {
        alert(err.message);
...
export async function register(userData) {
    try {
        ...
        const result = await response.json();
        setUserData(userData.username, result.sessionToken, result.objectId, userData['isTeam']);
    } catch (err) {
        alert(err.message);
        throw new Error(err.message);
    }
